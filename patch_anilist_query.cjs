...
const targetFuncStart = `  const anilistQuery = \`
  query ($search: String, $genre: String, $format: MediaFormat, $status: MediaStatus, $page: Int, $perPage: Int, $type: MediaType) {
    Page(page: $page, perPage: $perPage) {
      media(search: $search, type: $type, genre: $genre, format: $format, status: $status, sort: [\${sort}], isAdult: false, genreNotIn: ["Hentai"]) {`;
